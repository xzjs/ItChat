...
try:
    ...
    with open(pluginListDir) as f:
        pluginList = json.loads(f.read())
except:
    pluginList = {}
...
if 'QRCode' in pluginList['systemmodules']:
    try:
        ...
        sys_print('INFO',
                  'Command line QRCode loaded failed, if you want to use this, you need to run `pip install Image`')
...
# Test msgdealers.collect
if 'collect' in pluginList['msgdealers']:
    try:
        pluginList['msgdealers'].remove('collect')
        from plugin.msgdealers.collect import collect

        pluginList['msgdealers'].append('collect')
        sys_print('SUCC', 'msgdealers.collect')
    except Exception, e:
        sys_print('WARN', e.message)
