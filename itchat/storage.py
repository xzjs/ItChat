...
class Storage:
    ...
    def update_user(self, PYQuanPin, **kwargs):
        with Sqlite3Client(self.sqlDir) as s3c:
            ...
            try:
                s3c.insert_data('memberList',
                                items=[PYQuanPin, dataDict['NickName'], dataDict['UserName'], dataDict['Other']])
            except:
                return dataDict
        return True
    ...
    def store_report(self, userName, msg):
        with Sqlite3Client(self.sqlDir) as s3c:
            s3c.insert_data('report',
                            [int(time.time()), msg, userName])
