...
class GroupRobot:
    ...
    def get_nickname(self, groupName, userName):
        with Sqlite3Client(os.path.join(SQL_DIR, SQL_NAME)) as s3c:
            nickName = s3c.query('select * from memberlist where groupname=? and username=?', [groupName, userName])[0][2]
            return nickName
...
def grouptalking(msg, s, client, get_reply, replyToAll=False):
    ...
    msg['FromUserName']=gr.get_nickname(groupName,userName)
